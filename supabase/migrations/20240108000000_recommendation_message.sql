-- Add optional short message when sending a recommendation.

ALTER TABLE public.recommendations
  ADD COLUMN IF NOT EXISTS message TEXT;

COMMENT ON COLUMN public.recommendations.message IS 'Optional short message from sender to recipient.';

-- Recreate inbox RPC with message column (must DROP first when changing return type)
DROP FUNCTION IF EXISTS public.get_recommendations_received(UUID);

CREATE FUNCTION public.get_recommendations_received(p_user_id UUID)
RETURNS TABLE (
  id UUID,
  from_user_id UUID,
